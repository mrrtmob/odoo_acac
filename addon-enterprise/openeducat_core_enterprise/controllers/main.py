...
class SubjectRegistrationPortal(CustomerPortal):
    ...
    def get_search_domain_subject_registration(self, search, attrib_values):
        ...
        if search:
            for srch in search.split(" "):
                domain += [
                    '|', '|', '|', ('name', 'ilike', srch), ('batch_id', 'ilike', srch),
    ...
    def _prepare_portal_layout_values(self):
        ...
        student_id = request.env['op.student'].sudo().search(
            [('user_id', '=', user.id)])
        values = super(SubjectRegistrationPortal, self)._prepare_portal_layout_values()
        registartion_count = request.env['op.subject.registration'].sudo().search_count(
    ...
    def portal_student_subject_registration_list(
        ...
        domain += self.get_search_domain_subject_registration(search, attrib_values)
        if student_id:
        ...
        total = request.env['op.subject.registration'].sudo().search_count(domain)
        if student_id:
            pager = portal_pager(
